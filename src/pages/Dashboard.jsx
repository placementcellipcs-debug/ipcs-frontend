...
export default function Dashboard() {
  ...
  if (loading) {
    return (
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: '100vh', background: 'var(--bg-dark)', color: 'var(--accent-cyan)' }}>
        <i className="ph-bold ph-spinner animate-spin" style={{ fontSize: '3rem' }}></i>
        <p style={{ marginTop: '1rem', fontWeight: 600 }}>Syncing your dashboard...</p>
      </div>
    );
  }
  ...
  return (
    <div className="dashboard-layout-wrapper" style={{ display: 'flex', flexDirection: 'column', minHeight: '100vh', background: 'var(--bg-dark)' }}>
      
      {/* ==========================================
          TOP HEADER / NAVIGATION
      ========================================== */}
      <header style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '15px 25px', background: 'var(--card-bg)', borderBottom: '1px solid var(--card-border)', position: 'sticky', top: 0, zIndex: 10 }}>
        <div style={{ display: 'flex', alignItems: 'center' }}>
          {activeTab !== 'dashboard' && (
            <button 
              type="button" 
              onClick={() => setActiveTab('dashboard')} 
              style={{ background: 'transparent', border: '1px solid var(--card-border)', color: 'var(--text-main)', padding: '6px 14px', borderRadius: '8px', fontWeight: 600, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '6px' }}
            >
              <i className="ph-bold ph-arrow-left"></i> Dashboard
            </button>
          )}
          <span style={{ marginLeft: '10px', fontSize: '0.95rem', fontWeight: 700, color: 'var(--text-main)' }}>
            {getBreadcrumb()}
          </span>
        </div>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '20px' }}>
          <i 
            ...
            style={{ fontSize: '1.4rem', cursor: 'pointer', color: 'var(--text-muted)', transition: 'color 0.2s' }}
            onClick={toggleTheme}
          ...
          <div 
            ...
            style={{ width: '42px', height: '42px', borderRadius: '50%', background: 'var(--accent-blue)', color: 'white', display: 'flex', justifyContent: 'center', alignItems: 'center', fontWeight: '800', fontSize: '1.2rem', cursor: 'pointer', boxShadow: '0 4px 10px rgba(59, 130, 246, 0.4)' }}
          >
      ...
      {/* ==========================================
          MAIN CONTENT AREA
      ========================================== */}
      <div className="dashboard-content" style={{ padding: '25px', flex: 1 }}>
        
        {/* TAB: DASHBOARD HOME */}
        {activeTab === 'dashboard' && (
          <div style={{ animation: 'fadeIn 0.3s ease-in-out' }}>
            <div className="dash-top-row" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '20px', marginBottom: '20px' }}>
              <div className="hero-banner" style={{ background: 'var(--card-bg)', padding: '25px', borderRadius: '16px', border: '1px solid var(--card-border)' }}>
                <div className="greeting-subtitle" style={{ fontSize: '0.9rem', color: 'var(--text-muted)', fontWeight: 600, textTransform: 'uppercase', marginBottom: '5px' }}>{greeting}</div>
                <h2 style={{ margin: '0 0 10px 0', fontSize: '1.8rem' }}>Welcome back, <span style={{ color: 'var(--accent-cyan)' }}>{user.name || 'Student'}</span>!</h2>
                <div className="full-date-subtext" style={{ fontSize: '0.85rem', color: 'var(--text-muted)' }}>
                  {new Date().toLocaleDateString('en-US', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' })}
                </div>
              </div>

              <div className="quick-actions-card" style={{ background: 'var(--card-bg)', padding: '25px', borderRadius: '16px', border: '1px solid var(--card-border)' }}>
                ...
                <div className="quick-actions-grid" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px', marginTop: '15px' }}>
                  <div className="qa-btn" onClick={() => setActiveTab('talentino')} style={{ display: 'flex', alignItems: 'center', gap: '10px', background: 'var(--bg-dark)', padding: '10px', borderRadius: '10px', cursor: 'pointer', color: 'var(--text-main)', border: '1px solid var(--card-border)' }}><div style={{ padding: '8px', background: 'rgba(168, 85, 247, 0.1)', color: '#a855f7', borderRadius: '8px' }}><i className="ph-fill ph-check-circle" style={{ fontSize: '1.25rem' }}></i></div> Talentino</div>
                  <div className="qa-btn" onClick={() => setTpoModal(true)} style={{ display: 'flex', alignItems: 'center', gap: '10px', background: 'var(--bg-dark)', padding: '10px', borderRadius: '10px', cursor: 'pointer', color: 'var(--text-main)', border: '1px solid var(--card-border)' }}><div style={{ padding: '8px', background: 'rgba(34, 197, 94, 0.1)', color: '#22c55e', borderRadius: '8px' }}><i className="ph-fill ph-address-book" style={{ fontSize: '1.25rem' }}></i></div> Contact TPO</div>
                  <div className="qa-btn" onClick={() => setHelpModal(true)} style={{ display: 'flex', alignItems: 'center', gap: '10px', background: 'var(--bg-dark)', padding: '10px', borderRadius: '10px', cursor: 'pointer', color: 'var(--text-main)', border: '1px solid var(--card-border)' }}><div style={{ padding: '8px', background: 'rgba(239, 68, 68, 0.1)', color: '#ef4444', borderRadius: '8px' }}><i className="ph-fill ph-headset" style={{ fontSize: '1.25rem' }}></i></div> Request Help</div>
                  <div className="qa-btn" onClick={() => setActiveTab('profile')} style={{ display: 'flex', alignItems: 'center', gap: '10px', background: 'var(--bg-dark)', padding: '10px', borderRadius: '10px', cursor: 'pointer', color: 'var(--text-main)', border: '1px solid var(--card-border)' }}><div style={{ padding: '8px', background: 'rgba(56, 189, 248, 0.1)', color: '#38bdf8', borderRadius: '8px' }}><i className="ph-fill ph-user" style={{ fontSize: '1.25rem' }}></i></div> Profile</div>
                </div>
              </div>
            </div>
        ...
        {activeTab === 'guide' && (
          <div style={{ animation: 'fadeIn 0.3s ease-in-out' }}>
            ...
            <div className="resume-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '1.5rem' }}>
              {guideResources.map((resource, idx) => (
                <div key={idx} className="resume-card" onClick={() => window.open(resource.link, '_blank')} style={{ background: 'var(--card-bg)', padding: '20px', borderRadius: '12px', border: '1px solid var(--card-border)', display: 'flex', alignItems: 'center', gap: '15px', cursor: 'pointer' }}>
                  <div className="resume-icon-box" style={{ background: resource.bg, color: resource.color, width: '50px', height: '50px', borderRadius: '12px', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '1.5rem' }}>
        ...
        {activeTab === 'settings' && (
          <div style={{ animation: 'fadeIn 0.3s ease-in-out' }}>
            ...
            <div className="settings-container" style={{ display: 'flex', gap: '2rem', flexWrap: 'wrap' }}>
              <div className="settings-sidebar" style={{ display: 'flex', flexDirection: 'column', gap: '10px', minWidth: '250px' }}>
                <div className={`settings-tab ${settingsTab === 'security' ? 'active' : ''}`} onClick={() => setSettingsTab('security')} style={{ padding: '12px 20px', borderRadius: '8px', cursor: 'pointer', background: settingsTab === 'security' ? 'var(--accent-blue)' : 'var(--card-bg)', color: settingsTab === 'security' ? '#fff' : 'var(--text-main)', border: '1px solid var(--card-border)' }}>
                  <i className="ph-fill ph-lock-key"></i> Security
                </div>
                <div className={`settings-tab ${settingsTab === 'appearance' ? 'active' : ''}`} onClick={() => setSettingsTab('appearance')} style={{ padding: '12px 20px', borderRadius: '8px', cursor: 'pointer', background: settingsTab === 'appearance' ? 'var(--accent-blue)' : 'var(--card-bg)', color: settingsTab === 'appearance' ? '#fff' : 'var(--text-main)', border: '1px solid var(--card-border)' }}>
                  <i className="ph-fill ph-palette"></i> Appearance
                </div>
              </div>

              <div className="settings-content-area" style={{ flex: 1, background: 'var(--card-bg)', padding: '25px', borderRadius: '16px', border: '1px solid var(--card-border)' }}>
                ...
                {settingsTab === 'appearance' && (
                  <div style={{ animation: 'fadeIn 0.3s ease-in-out' }}>
                    ...
                    <div style={{ display: 'flex', gap: '15px' }}>
                      <div className={`theme-option-box ${theme === 'light' ? 'selected' : ''}`} onClick={() => { setTheme('light'); document.body.setAttribute('data-theme', 'light'); }} style={{ flex: 1, padding: '15px', border: `2px solid ${theme === 'light' ? 'var(--accent-blue)' : 'var(--card-border)'}`, borderRadius: '12px', cursor: 'pointer', textAlign: 'center' }}>
                        <div style={{ fontSize: '1rem', fontWeight: 700, color: 'var(--text-main)' }}>☀️ Light</div>
                      </div>
                      <div className={`theme-option-box ${theme === 'dark' ? 'selected' : ''}`} onClick={() => { setTheme('dark'); document.body.setAttribute('data-theme', 'dark'); }} style={{ flex: 1, padding: '15px', border: `2px solid ${theme === 'dark' ? 'var(--accent-blue)' : 'var(--card-border)'}`, borderRadius: '12px', cursor: 'pointer', textAlign: 'center' }}>
                        <div style={{ fontSize: '1rem', fontWeight: 700, color: 'var(--text-main)' }}>🌙 Dark</div>
                      </div>
                    </div>
                  </div>
                )}
      ...
      {/* SIDE DRAWER */}
      <div className={`drawer-overlay ${drawerOpen ? 'open' : ''}`} style={{ position: 'fixed', top: 0, left: 0, width: '100vw', height: '100vh', background: 'rgba(0,0,0,0.6)', backdropFilter: 'blur(5px)', zIndex: 1000, display: drawerOpen ? 'block' : 'none' }} onClick={(e) => { if (typeof e.target.className === 'string' && e.target.className.includes('drawer-overlay')) setDrawerOpen(false); }}>
        <div className="drawer-card" style={{ position: 'absolute', right: 0, top: 0, width: '320px', height: '100vh', background: 'var(--card-bg)', display: 'flex', flexDirection: 'column', boxShadow: '-5px 0 25px rgba(0,0,0,0.5)', animation: 'slideInRight 0.3s ease-out' }}>
          <div className="drawer-header" style={{ background: 'var(--accent-blue)', color: '#ffffff', padding: '2.5rem 1.5rem 1.5rem 1.5rem', position: 'relative' }}>
            <div className="drawer-close-btn" style={{ position: 'absolute', top: '1rem', right: '1rem', width: '32px', height: '32px', borderRadius: '50%', border: '1px solid rgba(255, 255, 255, 0.4)', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', fontSize: '1.2rem' }} onClick={() => setDrawerOpen(false)}>
              <i className="ph-bold ph-x"></i>
            </div>
            <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
              <div style={{ width: '60px', height: '60px', borderRadius: '50%', background: '#ffffff', color: '#3b82f6', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 800, fontSize: '1.5rem', border: '2px solid rgba(255,255,255,0.3)', overflow: 'hidden' }}>
          ...
          <div style={{ flex: 1, padding: '1rem 0', overflowY: 'auto' }}>
            <div className="drawer-item" style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '1rem 1.5rem', borderBottom: '1px solid var(--card-border)', color: 'var(--text-main)', fontSize: '0.95rem', fontWeight: 600, cursor: 'pointer' }} onClick={() => { setActiveTab('dashboard'); setDrawerOpen(false); }}><div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}><i className="ph-fill ph-house" style={{ color: 'var(--text-muted)', fontSize: '1.2rem' }}></i> Dashboard</div><span>&rsaquo;</span></div>
            <div className="drawer-item" style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '1rem 1.5rem', borderBottom: '1px solid var(--card-border)', color: 'var(--text-main)', fontSize: '0.95rem', fontWeight: 600, cursor: 'pointer' }} onClick={() => { setActiveTab('talentino'); setDrawerOpen(false); }}><div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}><i className="ph-fill ph-user-check" style={{ color: 'var(--text-muted)', fontSize: '1.2rem' }}></i> Talentino</div><span>&rsaquo;</span></div>
            <div className="drawer-item" style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '1rem 1.5rem', borderBottom: '1px solid var(--card-border)', color: 'var(--text-main)', fontSize: '0.95rem', fontWeight: 600, cursor: 'pointer' }} onClick={() => { setActiveTab('profile'); setDrawerOpen(false); }}><div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}><i className="ph-fill ph-user" style={{ color: 'var(--text-muted)', fontSize: '1.2rem' }}></i> Profile</div><span>&rsaquo;</span></div>
            <div className="drawer-item" style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '1rem 1.5rem', borderBottom: '1px solid var(--card-border)', color: 'var(--text-main)', fontSize: '0.95rem', fontWeight: 600, cursor: 'pointer' }} onClick={() => { setActiveTab('status'); setDrawerOpen(false); }}><div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}><i className="ph-fill ph-list-checks" style={{ color: 'var(--text-muted)', fontSize: '1.2rem' }}></i> Application Status</div><span>&rsaquo;</span></div>
            <div className="drawer-item" style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '1rem 1.5rem', borderBottom: '1px solid var(--card-border)', color: 'var(--text-main)', fontSize: '0.95rem', fontWeight: 600, cursor: 'pointer' }} onClick={() => { setActiveTab('vacancies'); setDrawerOpen(false); }}><div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}><i className="ph-fill ph-briefcase" style={{ color: 'var(--text-muted)', fontSize: '1.2rem' }}></i> Current Job Vacancies</div><span>&rsaquo;</span></div>
            <div className="drawer-item" style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '1rem 1.5rem', borderBottom: '1px solid var(--card-border)', color: 'var(--text-main)', fontSize: '0.95rem', fontWeight: 600, cursor: 'pointer' }} onClick={() => { setActiveTab('guide'); setDrawerOpen(false); }}><div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}><i className="ph-fill ph-book-open" style={{ color: 'var(--text-muted)', fontSize: '1.2rem' }}></i> Guide</div><span>&rsaquo;</span></div>
            <div className="drawer-item" style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '1rem 1.5rem', borderBottom: '1px solid var(--card-border)', color: 'var(--text-main)', fontSize: '0.95rem', fontWeight: 600, cursor: 'pointer' }} onClick={() => setTpoModal(true)}><div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}><i className="ph-fill ph-address-book" style={{ color: 'var(--text-muted)', fontSize: '1.2rem' }}></i> Contact TPO</div><span>&rsaquo;</span></div>
            <div className="drawer-item" style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '1rem 1.5rem', borderBottom: '1px solid var(--card-border)', color: 'var(--text-main)', fontSize: '0.95rem', fontWeight: 600, cursor: 'pointer' }} onClick={() => setHelpModal(true)}><div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}><i className="ph-fill ph-info" style={{ color: 'var(--text-muted)', fontSize: '1.2rem' }}></i> Request Help</div><span>&rsaquo;</span></div>
            <div className="drawer-item" style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '1rem 1.5rem', borderBottom: '1px solid var(--card-border)', color: 'var(--text-main)', fontSize: '0.95rem', fontWeight: 600, cursor: 'pointer' }} onClick={() => { setActiveTab('settings'); setDrawerOpen(false); }}><div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}><i className="ph-fill ph-gear" style={{ color: 'var(--text-muted)', fontSize: '1.2rem' }}></i> Settings</div><span>&rsaquo;</span></div>
          </div>
        ...
        <div className="report-modal-overlay">
          <div className="report-card" style={{ maxWidth: '400px', textAlign: 'center' }}>
            ...
            <div style={{ display: 'flex', gap: '10px', justifyContent: 'center' }}>
              <button className="btn-action" style={{ flex: 1, background: '#2563eb', border: 'none' }}><i className="ph-fill ph-phone"></i> Call</button>
              <button className="btn-action" style={{ flex: 1, background: '#22c55e', border: 'none' }}><i className="ph-fill ph-whatsapp-logo"></i> WhatsApp</button>
            </div>
          </div>
        </div>
      )}
